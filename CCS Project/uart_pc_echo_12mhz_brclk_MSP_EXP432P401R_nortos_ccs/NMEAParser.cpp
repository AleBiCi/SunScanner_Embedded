...
bool MessageRMC::parseRMC() {
    ...
    char* input_mess;
        ...
        while ((tok_ptr = my_strsep(&input_mess, ",")) != nullptr) {
            ...
            if(strlen(tok_ptr)!=0) {
                //tokens.push_back(tok_ptr);
                //strcpy(tokens[iter], tok_ptr);
                tokens[iter] = strdup(tok_ptr);
            } else {
    ...
    if (!validateChecksum() || strcmp(&this->status, "A")!=0) {
        error = true; // if checksum comparison and validity status check didn't give positive result, return TRUE
    } else return false;
    ...
    if(strlen(tokens[9])!=0) {
        ...
        tmp[2] = '\0';
        this->time.tm_mon = std::strtol(tmp, nullptr, 10); // Month is [0-11]
        strcpy(tmp, "");

            // Year
        strncpy(tmp, tokens[9]+4, 2);
        tmp[2] = '\0';
        this->time.tm_year = std::strtol(tmp, nullptr, 10) + 2000; // Year is from 2000
        strcpy(tmp, "");
    }
