...
#ifndef __NMEAPARSER_HPP__
#define __NMEAPARSER_HPP__
...
public:
    ...
    MessageNMEA();
    MessageNMEA(char* mess);
    ~MessageNMEA();

    void setMessage(char* mess);
};

class MessageRMC : public MessageNMEA {
public:
    /* 0 */ char* message_ID;
    ...
    /* 12 */ char mode;
    /* 13 */ char* checksum; // 2 hexadecimal (0-F) values

    MessageRMC();
    MessageRMC(char* mess);
    ~MessageRMC();

    bool validateChecksum();    // validates the checksum (XOR of all chars between '$' and '*' including ',')
    bool parseRMC(); // parses the RMC sentences (if valid) and assigns values to struct attributes; returns result of complete validation

    void printSent();
};


#endif // __NMEAPARSER.HPP__
